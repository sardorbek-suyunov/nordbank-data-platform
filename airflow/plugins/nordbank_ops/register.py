...
def write_reconciliation(
    ...
    *,
    ...
    """Record one batch's contribution to one source day.

    Per batch rather than per day. Keyed per day and upserted, two batches covering one day
    fight over the row and the last one wins, which on the first complete backfill let a
    narrow re-run after a drift halt overwrite nine entities' day counts with the handful of
    rows its tail-of-the-day window had seen. The day is the sum of its batches, and
    `ops.source_reconciliation_daily` is where that sum lives.

    The conflict clause is still needed, because a retry of the same batch writes the same key
    again and should replace its own contribution rather than add to it.
    """
    connection.execute(
        """
        insert into ops.source_reconciliation (
            source_system, entity, source_date, batch_id, rows_claimed, rows_landed,
            rows_quarantined, recorded_at
        ) values (?, ?, ?, ?, ?, ?, ?, ?)
        on conflict (source_system, entity, source_date, batch_id) do update set
