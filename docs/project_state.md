...
**M0, M1 and M2 are complete.** The source system has a schema and a coherent operating history in it.
...
- A deterministic generator in `generator/`, loaded by `COPY`. Three inputs fix the output
  ...
  generates and loads 10.6 s and the `dev` profile 322 s, and the committed `ci`
  manifest is regenerated by CI and compared, so determinism is enforced rather than claimed.
...
What does not exist yet: the mutation engine, any ingestion, the dbt project, any bronze,
silver or gold model, the data contracts, the quality framework, the governance work, Power
BI, Streamlit, Terraform and the BigQuery target.
...
| `make seed` | 209,190 rows at `ci` in 11.5 s; 11,468,955 at `dev` in 328 s, 2,321,273 of them transactions |
| `make seed-verify` | 14 invariants, all pass; invariant 9's statistical band is not asserted at `ci` and says so |
...
M3 is the mutation engine: it advances the source by one business day, producing updates,
soft deletes, late arrivals and schema drift for M4 to ingest. The pattern it needs is in the
runbook — load with an anchor in the past, then step forward, so a backfill has genuine
